...
export const Capsules_menu = async () => {
...
// Llama a la función para generar el menú de cápsulas al cargar la página
Capsules_menu();
...
export const capsulesmaquetacion  = async(index = 0)=>{
    ...
    let launchText = launchCount === 1 ? "1 resultado's" : `${launchCount} resultado`;

    let water_landings = capsule.water_landings;
    let land_landings = capsule.land_landings;

    let launches1 = capsule.launches;
    let launchCount1 = launches1.length;
    let launchText1 = launchCount === 1 ? (launchCount) : `${launchCount1} resultado`;

    let typeC = capsule.type;
